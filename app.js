...
async function menuInventario() {
    ...
    while (!volver) {
        ...
        if (opcion === '1') {
            ...
            inventario.agregar({ producto, categoria, stock: Number(stock) || 0 });
...
async function menuProveedores() {
    ...
    while (!volver) {
        ...
        if (opcion === '1') {
            ...
            proveedores.agregar({ nombre, producto, telefono });
            ...
            const id = await rl.question('ID a eliminar: ');
            proveedores.eliminar(Number(id));
